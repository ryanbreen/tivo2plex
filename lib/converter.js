...
const makeRecordSafe = async (f) => {
    
    const original_directory = dirname(f)
    const original_filename = basename(f, '.txt')

    // Rename the parts if the path has a comma in it
    if (original_filename.indexOf(',') !== -1) {
        // Move pytivo file and actual file
        const new_name = original_filename.replace(',', '')
        console.log(`${original_filename} => ${new_name}`)
        await promises.rename(`${original_directory}/${original_filename}`, `${original_directory}/${new_name}`)
        await promises.rename(`${original_directory}/${original_filename}.txt`, `${original_directory}/${new_name}.txt`)
        return `${original_directory}/${new_name}.txt`
    }
