...
import { ROOT_DIR, PLEX_DIR } from './config.js'
...
const cleanupAfterRun = async (record) => {
    ...
    try {
        ...
        await promises.rm(record.fully_qualified_path_no_extension + "_ffmpeg", { recursive: true }).catch(errored)
    } catch(e) {
        console.log(e)
    }
}

const processFile = async (segment, f) => {
